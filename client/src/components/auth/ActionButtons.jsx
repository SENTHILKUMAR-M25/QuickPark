...
export function SocialButton({ children, icon: Icon, onClick, disabled }) {
  return (
    <motion.button
      ...
      onClick={onClick}
      disabled={disabled}
      className="inline-flex w-full items-center justify-center gap-2.5 rounded-xl border border-slate-200 bg-white/80 px-5 py-3 text-sm font-semibold text-ink-700 transition-colors duration-200 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60 dark:border-ink-700 dark:bg-ink-900/60 dark:text-ink-100 dark:hover:bg-ink-800"
    >
      {Icon && <Icon size={18} />}
      <span className="flex items-center gap-1.5">
        {children}
        <span className="rounded-md bg-ink-100 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-400 dark:bg-ink-800 dark:text-ink-500">
          Soon
        </span>
      </span>
    </motion.button>
  ...
  return (
    <div className="relative my-6">
      <div className="absolute inset-0 flex items-center">
        <span className="w-full border-t border-slate-200 dark:border-ink-700" />
      </div>
      <div className="relative flex justify-center">
        <span className="bg-[#fbfcfe] px-4 text-xs font-medium uppercase tracking-wider text-slate-400 dark:bg-ink-950">
          {label}
        </span>
      </div>
    </div>
