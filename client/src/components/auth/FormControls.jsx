...
const fieldBase =
 "w-full rounded-xl border bg-white/70 px-4 py-3 text-sm text-ink placeholder:text-slate-400 outline-none transition-all duration-200:text-ink-500";
...
function Label({ children, htmlFor, required, hint }) {
 ...
 <label
 htmlFor={htmlFor}
 className="mb-1.5 flex items-center justify-between text-sm font-medium text-ink-700"
 >
 <span>
 {children}
...
export const PasswordInput = forwardRef(function PasswordInput(
 ...
 className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 transition-colors hover:text-ink:text-ink-100"
 >
...
export function Checkbox({ label, error, id, register, ...props }) {
 ...
 type="checkbox"
 id={id}
 className="peer h-5 w-5 cursor-pointer appearance-none rounded-md border border-slate-300 bg-white transition-all checked:border-brand-600 checked:bg-brand-600"
 {...register}
 {...props}
 />
 <Check
