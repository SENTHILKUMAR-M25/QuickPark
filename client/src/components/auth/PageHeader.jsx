...
export function AuthHeader({ title, subtitle }) {
  return (
    <div className="mb-8">
      <h1 className="font-display text-3xl font-bold tracking-tight text-ink dark:text-white sm:text-[2.1rem]">
        {title}
      </h1>
      {subtitle && (
        <p className="mt-2 text-sm leading-relaxed text-slate-500 dark:text-ink-400">{subtitle}</p>
      )}
    </div>
  );
}
  ...
  return (
    <p className="mt-8 text-center text-sm text-slate-500 dark:text-ink-400">
      ...
      {onToggle ? (
        <button
          onClick={onToggle}
          className="inline-flex items-center gap-0.5 font-semibold text-brand-600 transition-colors hover:text-brand-700 dark:text-brand-400 dark:hover:text-brand-300"
        >
          {linkText}
