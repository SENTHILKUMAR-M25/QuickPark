...
const DashboardPreview = memo(function DashboardPreview() {
 ...
 <Clock size={13} /> Upcoming
  ...
  <Link to="/user/dashboard" className="text-xs font-semibold text-white/80 hover:text-white">
 View all →
 </Link>
 </div>
 ...
 >
 <div className="flex items-center justify-between">
 <h3 className="font-display text-base font-bold text-ink">Recent Activity</h3>
 ...
 <h3 className="font-display text-base font-bold text-ink">Favorite Parking</h3>
 </div>
 <ul className="mt-4 space-y-3">
 {FAVORITES.map((f) => (
 <li key={f.name} className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3">
 <div>
 <p className="text-sm font-semibold text-slate-800">{f.name}</p>
 <p className="text-xs text-slate-400">{f.area}</p>
 </div>
 <span className="text-xs font-bold text-brand-600">{f.price}</span>
 </li>
 ))}
 </ul>
 </div>
