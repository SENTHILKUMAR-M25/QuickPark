...
import { dashboardFor } from "../../lib/auth.meta";
...
const VARIANTS = {
 ...
 ADMIN: {
 role: "ADMIN",
 header: { title: "Admin sign in", subtitle: "Manage the Quick Park marketplace." },
 panel: {
 headline: (
 <>
 Full control.
 <br />
 <span className="text-mint-300">Complete visibility.</span>
 </>
 ),
 subline: "Monitor users, providers, listings and revenue from one console.",
 statItems: LOGIN_STATS,
 },
...
export default function LoginPage({ role = "USER" }) {
 ...
 navigate(dashboardFor(res.user?.role), { replace: true });
 } catch (err) {
 ...
 className="text-sm font-semibold text-brand-600 transition-colors hover:text-brand-700:text-brand-300"
 >
 Forgot password?
 </a>
 </div>
