  ...
  return (
    ...
    >
      ...
      <div className="mt-4 text-center text-sm">
        {timer > 0 ? (
          <p className="text-slate-500 dark:text-ink-400">
            Resend in <span className="font-semibold tabular-nums text-ink dark:text-white">{format(timer)}</span>
          </p>
        ) : (
          <button
            onClick={resend}
            disabled={sendingAgain}
            className="font-semibold text-brand-600 transition-colors hover:text-brand-700 disabled:opacity-60 dark:text-brand-400"
          >
